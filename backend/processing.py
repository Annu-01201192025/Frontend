...
# TODO before final submission: replace with exact numbers from the DGCA portal.
DGCA_ROUTE_WEIGHTS = {
    "DEL-BOM": 0.42,
    "BLR-DEL": 0.33,
    "MAA-DEL": 0.25,
...
def booking_window(record: dict) -> int:
    """
    Classify a record into the MoSPI-standard booking-window bucket (7, 15,
    or 30 days out) from departure_time - scraped_at, snapped to the nearest
    bucket (a real scrape will never land on exactly 7.000 days out).

    Falls back to the 7-day bucket if the timestamps are missing/unparseable,
    since that matches Member 3's current fixed 7-day scraper anyway.
    """
    try:
        scraped = datetime.fromisoformat(record["scraped_at"])
        departs = datetime.fromisoformat(record["departure_time"])
...
if __name__ == "__main__":
    ...
    print("\nSTEP 2: detect_anomalies()  [history: DEL-BOM 7-day avg = 5000.0]")
    flagged = detect_anomalies(cleaned, history={"DEL-BOM": [4900, 5000, 5100, 4950, 5050, 5000, 4900]})
    for r in flagged:
...
def calculate_historical_index(
    ...
    base_prices_dict: dict[str, float],
    weights: dict[str, float] = DGCA_ROUTE_WEIGHTS,
) -> list[dict]:
    """
    Calculate the daily Laspeyres Airfare Price Index from historical
    route-level average fares.
    """
    ...
    for row in history_rows:
        ...
        if route not in weights or route not in base_prices_dict:
            continue

        by_date[row["date"]][route] = float(row["average_price"])
    ...
    for date in sorted(by_date):
        ...
        for route, weight in weights.items():
            current_price = by_date[date].get(route)
            base_price = base_prices_dict.get(route)
        ...
        if weight_total:
            overall_index = round(
                BASE_INDEX * (weighted_sum / weight_total),
                2
            )
